...
Listing calls go through APISIX to the shared Envoy gRPC-JSON transcoder. APISIX has one listing route, `/listings/*`; the REST path to gRPC method mapping lives in `proto/listing.proto` through `google.api.http` annotations.

```bash
curl -i \
  'http://localhost:9080/listings/availability?eventId=event-3' \
  -H "authorization: Bearer $TOKEN"
```

```bash
curl -i \
  -X POST http://localhost:9080/listings/reserve \
  -H "authorization: Bearer $TOKEN" \
  -H 'content-type: application/json' \
  -d '{"eventId":"event-3","seats":1}'
```

```bash
curl -i \
  -X POST http://localhost:9080/listings/release \
  -H "authorization: Bearer $TOKEN" \
  -H 'content-type: application/json' \
  -d '{"eventId":"event-3","seats":1}'
```
...
The public HTTP routes are defined in `apisix/declarative.yaml`:

- `POST /orders` maps to `order.OrderService.PlaceOrder`
- `/listings/*` maps to the shared Envoy gRPC-JSON transcoder; `proto/listing.proto` annotations define the listing REST path mapping
- APISIX still uses `grpc-transcode` for the order route
